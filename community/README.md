# Miri Standard Community

Welcome to the Miri Standard community! Just as Miranda Sharifi used her enhanced capabilities to serve the broader
community rather than isolation, our project aims to enhance collaboration between human developers and autonomous
agents for everyone's benefit.

This directory contains information about how our community is organized and how to participate in building the future
of agent-friendly Python packaging.

## Community Structure

### Special Interest Groups (SIGs)

Our community is organized into Special Interest Groups that focus on specific aspects of the project:

- **[SIG-Community](sig-community/)**: Community building, outreach, and governance
- **[SIG-Spec](sig-spec/)**: Standards development and specification work

### Working Groups

Working groups are formed for specific, time-bounded initiatives. Currently active working groups:

*(No active working groups at this time).*

## Communication

### Regular Meetings

- **Community Meeting**: [Schedule to be determined]
- **SIG Meetings**: See individual SIG directories for schedules

### Online Channels

- **GitHub Discussions**: For general questions and community discussions
- **GitHub Issues**: For bug reports, feature requests, and technical discussions
- **[Additional channels to be added]**

## Getting Involved

### New Contributors

1. Read our [Code of Conduct](../CODE-OF-CONDUCT.md)
2. Review the [Contributing Guidelines](../CONTRIBUTING.md)
3. Join a SIG that matches your interests
4. Attend community meetings
5. Start contributing!

### Areas of Contribution

- **Standards Development**: Help develop and refine specifications
- **Documentation**: Improve project documentation and guides
- **Community Building**: Help grow and support the community
- **Implementation**: Create reference implementations and examples
- **Testing**: Develop test suites and validation tools
- **Outreach**: Present at conferences, write blog posts, create tutorials

## Community Calendar

[Calendar link to be added when available]

## Meeting Notes

All meeting notes are stored in the `meetings/` directory *(to be created)* and are organized by date and meeting type.

## Contact

- **Community Questions**: Use GitHub Discussions
- **Private Matters**: Contact a maintainer directly through GitHub; for security issues, use the repository's
  private vulnerability reporting (Security tab → Report a vulnerability)
- **Code of Conduct Issues**: See `CODE-OF-CONDUCT.md`; reports can be raised privately with a maintainer via GitHub

## Resources

- [Project Governance](../GOVERNANCE.md)
- [Project Maturity](../MATURITY.md)
- [Contributing Guidelines](../CONTRIBUTING.md)
- [Code of Conduct](../CODE-OF-CONDUCT.md)
