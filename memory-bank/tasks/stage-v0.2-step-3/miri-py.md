...
- [ ] **Publish miri-py to PyPI** (even a `0.2.0rc`). Fixes `pip install miri-py`, turns the standard's
      ...
      Packaging)_
- [ ] **Re-pin the vendored checks mirror to ≥ `4e51dd6`.** `PROVENANCE.json` pins a commit that predates the standard's
...
- [ ] **Purge "committee" from miri-py code** — `score_cmd.py:301` ("40 committee-defined checks"), `execution.py`,
...
- [ ] **`miri generate --output-dir <path>` crashes** (`unsupported operand type(s) for /: 'str' and 'str'` — needs
...
- [ ] **`miri build --generate-metadata` is a no-op** — the build report shows "Generate Metadata: No"; the wheel keeps
      the committed stamps, so MIRI-PY-011 still fires. _(same)_
- [ ] **`miri generate` omits `migration-guide.json`** (not derivable from a single version's source) — a
...
- [ ] **CLI-013** — argument-parse errors bypass the JSON envelope. `miri --json score --bogus` → exit 2, prose on
      ...
      Author)_
- [ ] **CLI-040 producer half** — miri-py already builds `danger`/`mutates` in `describe.py` but strips them unless
...
- [ ] **§5.4 CLI-exclusion violation** — `sdk-manifest.json` `api_index` contains the console-script target `main`
...
- [ ] **MIRI-PY-011 tolerance**: change `constants.py` from 26h to **24h** to match the canonical check YAML.
      _(Packaging)_
...
- [ ] `miri build` "Next Steps" banner still advertises the deprecated `miri lint` and globs `*-miri-*.whl` (matches
      nothing). _(CLI)_
- [ ] **examples-index.json orphan** — the generator emits it with a `$schema` to a nonexistent schema; delete
      `generate_examples_index` and standardize on the dist-info `AGENT_EXAMPLES.json`, or spec+check it. _(Agent-Tooling)_
- [ ] Fix the generator `$schema` domain (`generator.py`, `patterns_builder.py`) from `miri-standard.org` to
