...
files, so a mistake here propagates to every downstream tool. The governing schema is `schemas/check-v3.json`
...
1. Read `schemas/check-v3.json` and two or three sibling checks in the same category for tone and field patterns.
