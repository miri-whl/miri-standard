...
2. **Every check YAML validates against `schemas/check-v3.json`**, and the schema itself is a valid draft-07
   schema:
   ...
   schema = json.load(open('schemas/check-v3.json'))
...
`standards/*/checks/`, then adding a required field to `check-v3.json` breaks that vendored mirror until it re-syncs.
