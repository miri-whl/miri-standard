...
| --- | --- | --- |
| **Python wheels** | `agent-metadata/` in the wheel, `lifecycle.json`, 43 checks | Developed (0.3-draft) |
| **Command-line tools** | `--describe`, `check-update`, `changelog --since`, per-flag lifecycle, 43 checks | Developed (0.2-draft) |
| **Consumption** | What a consumer and a surface must do with the metadata: 18 `MIRI-SURFACE` + 18 `MIRI-CONSUMER` checks | Developed (0.3.1 / 0.5.0-draft) |
